...
% \begin{subequations}
%     \begin{align}
%         \text{max} \quad &\sum_{t\in T}\left(P_t^{DA} \cdot x_{t} + \mathbb{E}\left[P_t^{RT}(\xi) \cdot y^+_{t}(\xi) - P_t^{PN} \cdot y^-_{t}(\xi) + \rho_t(d) \cdot \left(d^+_{t}(\xi) - d^-_{t}(\xi)\right)\right]\right)&\\   
%         \text{s.t.} \quad &R_{t}(\xi) - x_{t} = y_{t}^{+}(\xi) - y_{t}^{-}(\xi) + z^C_{t}(\xi) - z^D_{t}(\xi) + d^+_{t}(\xi) - d^-_{t}(\xi)&\\
