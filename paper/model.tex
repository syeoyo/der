...
\begin{align*}
    ...
    &&\\
    &e^+_{it}(\xi) = y^+_{it}(\xi) - \sum_{j \in I}d^{+}_{ijt}(\xi) &\\
    &e^-_{it}(\xi) = y^-_{it}(\xi) - d^{-}_{it}(\xi) &\\
    &e^C_{it}(\xi) = z^C_{it}(\xi) - \sum_{j \in I}\widehat{d^{C}_{ijt}}(\xi) + \widecheck{d^{C}_{it}}(\xi) &\\
    &e^D_{it}(\xi) = z^D_{it}(\xi) - \widehat{d^{D}_{it}}(\xi) + \sum_{j \in I}\widecheck{d^{D}_{ijt}}(\xi) &\\
    &e^C_{it}(\xi) \leq K_i - z_{it}(\xi), \quad e^D_{it}(\xi) \leq z_{it}(\xi)&\\
    &z_{i,t+1}(\xi) = z_{it}(\xi) + e^C_{it}(\xi) - e^D_{it}(\xi) &\\
    &&\\
    ...
    &\sum_{j \in I}d^+_{ijt}(\xi) - \sum_{j \in I}y^-_{jt}(\xi)\leq d^+_{iit}(\xi)&\\
    &d^+_{iit}(\xi) \leq \widecheck{d^{C}_{it}}(\xi) + \widehat{d^{D}_{it}}(\xi) &\\
    &d^-_{it}(\xi) - \widehat{d^{C}_{iit}}(\xi) - \widecheck{d^{D}_{iit}}(\xi) \geq d^+_{jit}(\xi) + \widehat{d^{C}_{jit}}(\xi) + \widecheck{d^{D}_{jit}}(\xi) \quad \forall j \in I, j \neq i &\\
    ...
    & \sum_{i \in I} R_{it}(\xi) - \alpha_t = \beta^+_t(\xi) - \beta^-_t(\xi) &\\
