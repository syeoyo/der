...
\begin{subequations}
    ...
    &\left(K_i - z_{it}(\xi) - z^c_{it}(\xi)\right) - \left(d^+_{it}(\xi) - \sum_{i \in I}y^-_{it}(\xi)\right) \leq M_1(1-q^6_{it}(\xi))-\epsilon&\\
    & \left(d^+_{it}(\xi) - \sum_{i \in I}y^-_{it}(\xi) \right) - e^C_{it}(\xi) \leq M_1 q^6_{it}(\xi)&\\
    &\left(d^+_{it}(\xi) - \sum_{i \in I}y^-_{it}(\xi) \right) - \left(K_i - z_{it}(\xi)\right) \leq M_1(1-q^7_{it}(\xi))-\epsilon &\\
    &\left(K_i - z_{it}(\xi)\right) - e^C_{it}(\xi) \leq M_1 q^7_{it}(\xi)&
    \end{align*}
\end{subequations}
