"""ADMM benchmark for stochastic DER aggregation coordination.

This module mirrors the continuous centralized model in ``0527.ipynb`` and
adds a DER-wise consensus ADMM decomposition for the internal-market balance
constraint

    sum_i dp[i,t,s] == sum_i dm[i,t,s].

The ADMM benchmark is intentionally a comparison baseline: it solves the same
stochastic coordination problem, but recovers the coordination prices through
iterative dual updates instead of extracting them once from a centralized solve.
"""
...
class LocalDERModel:
    ...
    def set_admm_objective(self, target: np.ndarray) -> None:
        ...
        for t, s in product(range(self.t_count), range(self.s_count)):
            ...
            # Existing tiny regularization from 0527.ipynb.
            reg_coeff = float(-self.eps / self.s_count)
            quad_coeffs.extend([reg_coeff, reg_coeff])
            quad_v1.extend([self.dp[t, s], self.dm[t, s]])
            quad_v2.extend([self.dp[t, s], self.dm[t, s]])
...
def run_admm(
    ...
    for iteration in range(1, max_iter + 1):
        ...
        reg = regularization_value(local_solution["dp"], local_solution["dm"], eps)
...
def run_overnight_scalability(
    ...
    eps: float = 1e-8,
    level: str = "high",
    output_csv: str | Path = "admm_overnight_results.csv",
    average_csv: str | Path | None = "admm_overnight_seed_average.csv",
